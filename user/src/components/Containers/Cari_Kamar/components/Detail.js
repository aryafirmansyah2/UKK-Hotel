...
const Detail = () => {
    ...
    const getLengthKamar = () => {
        if (typeof window !== 'undefined') {
            axios.get(`http://localhost:8080/filter-room/${id}?in=${sessionStorage.getItem("check_in")}&out=${sessionStorage.getItem("check_out")}`)
                .then((result) => {
                    console.log(result.data)
                    setTersedia(result.data.room[0]?.kamar.length)
                })
        }
    }
    ...
    const handleSubmit = () => {
        if (jumlahKamar > 0) {
            if (jumlahKamar < tersedia) {
                ...
                Swal.fire({
                    title: 'Jumlah Kamar melebihi batas',
                    text: 'tolong isi kembali ',
                    icon: 'error',
                            ...
                            <div className='mt-3 '>
                                ...
                                <div className='w-full mt-5 pb-[25px] border-b-2 border-solid border-gray-300 dark:border-white'>
                                    ...
                                    <div className='relative flex items-center justify-end'>
                                        <input placeholder='Insert Jumlah Kamar' id="nomorKamar" type="number" name="jumlahKamar" min={0} max={tersedia} value={jumlahKamar || ''} onChange={handleChange} className='block w-full px-4 py-3 text-base text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500' required />
                                        <div className='absolute mr-12 font-semibold text-gray-400 '>Max {tersedia}</div>
                                    </div>
                                </div>
