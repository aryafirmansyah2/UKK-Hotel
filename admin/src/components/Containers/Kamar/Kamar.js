          ...
          <tbody>
            {kamars ? kamars.map((item, i) => (
              <tr key={i} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <td className="px-6 py-4">
                  {numberFormat(item.tipe_kamar?.harga)}
                </td>
                <td className="flex gap-5 px-6 py-4 ">
                  {/* <a href={'/kamar/detail_kamar/' + item.id_kamar}><RiEdit2Line className='text-lg hover:text-primary-800' /></a> */}
                  <button onClick={() => hapus(item.id_kamar)}><RiDeleteBin6Line className='text-lg hover:text-red-700' /></button>
                </td>
              </tr>
            )) :
              <tr>
