...
const EditTipeKamar = () => {
  ...
  const updateTipeKamar = async (e) => {
    if (e && e.preventDefault) {
      ...
      await axios.put(`http://localhost:8080/tipe_kamar/${id}`, formData)
        .then(() => {
          setTimeout(() => {
        ...
        <form onSubmit={updateTipeKamar}>
          <div className="grid gap-6 mt-5 mb-6 md:grid-cols-1" >
            ...
            <div>
              ...
              <input type="text" id="company" className="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Flowbite" value={harga} required />
            </div>
            <div>
